...
- Явно определенный конструктор. Такой конструктор реализуется по формату функции, его имя – это имя класса.
  ...
  - С параметрами – создает объект и инициализирует члены данных значениями параметров.
  
```cpp
class MyClass {
int field;
public:
  MyClass(); // Явно определенный конструктор без параметров
  MyClass(int field); // Явно определенный конструктор c     параметрами
};
```
